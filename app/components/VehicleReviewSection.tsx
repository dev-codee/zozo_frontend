...
import { VehicleReview, getVehicleReviews, postVehicleReview } from "@/app/lib/api";
...
export default function VehicleReviewSection({
  ...
  const fetchReviews = async (pageToFetch: number) => {
    ...
    try {
      const data = await getVehicleReviews(vehicleId, pageToFetch, 6);
      if (pageToFetch === 1) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const newRev = await postVehicleReview({
        vehicleId,
        rating,
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="What did you like or dislike about this EV?"
                  rows={4}
