      ...
      try {
        ...
        if (res.ok && data.data) {
          ...
          setFormData({
            ...
            release_date: phone.release_date ? new Date(phone.release_date).toISOString().split('T')[0] : '',
            status: phone.status || 'available',
            images: phone.images || [],
            specs: {
              display: {
                size_inches: phone.specs?.display?.size_inches || '',
                resolution: phone.specs?.display?.resolution || '',
                type: phone.specs?.display?.type || '',
                refresh_rate_hz: phone.specs?.display?.refresh_rate_hz || '',
                protection: phone.specs?.display?.protection || '',
                peak_brightness_nits: phone.specs?.display?.peak_brightness_nits || ''
              },
              performance: {
                chipset: phone.specs?.performance?.chipset || '',
                cpu: phone.specs?.performance?.cpu || '',
                gpu: phone.specs?.performance?.gpu || '',
                ram_options_gb: phone.specs?.performance?.ram_options_gb?.join(', ') || '',
                storage_options_gb: phone.specs?.performance?.storage_options_gb?.join(', ') || '',
                expandable_storage: phone.specs?.performance?.expandable_storage || false
              },
              camera: {
                rear_summary: phone.specs?.camera?.rear_summary || '',
                front_summary: phone.specs?.camera?.front_summary || '',
                video_recording: phone.specs?.camera?.video_recording || ''
              },
              battery: {
                capacity_mah: phone.specs?.battery?.capacity_mah || '',
                charging_watts: phone.specs?.battery?.charging_watts || '',
                fast_charging: phone.specs?.battery?.fast_charging || false,
                wireless_charging: phone.specs?.battery?.wireless_charging || false
              },
              body: {
                height_mm: phone.specs?.body?.height_mm || '',
                width_mm: phone.specs?.body?.width_mm || '',
                thickness_mm: phone.specs?.body?.thickness_mm || '',
                weight_g: phone.specs?.body?.weight_g || '',
                materials: phone.specs?.body?.materials || '',
                water_resistance: phone.specs?.body?.water_resistance || ''
              },
              connectivity: {
                network: phone.specs?.connectivity?.network || '',
                sim: phone.specs?.connectivity?.sim || '',
                usb: phone.specs?.connectivity?.usb || '',
                bluetooth: phone.specs?.connectivity?.bluetooth || '',
                nfc: phone.specs?.connectivity?.nfc || false
              },
              os: phone.specs?.os || '',
              extra_specs: {
                ...EXTRA_SPEC_FIELDS.reduce((acc, field) => ({ ...acc, [field]: '' }), {}),
                ...(phone.specs?.extra_specs || {})
              }
            },
            prices: phone.prices || [],
            seo: phone.seo || { meta_title: '', meta_description: '' },
            is_published: phone.is_published || false
          });
        ...
        <div className="p-8 max-w-5xl mx-auto pb-24">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center space-x-4">
              ...
              <h2 className="text-3xl font-bold text-gray-900">Edit Mobile</h2>
            </div>
            <button
